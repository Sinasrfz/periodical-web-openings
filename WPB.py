...
# Display a Read Me button
if st.sidebar.button("Read Me"):
    st.subheader("README for Steel Beams with Elliptically-Based Web Openings GUI")
    st.markdown("""
    **How to Use**:
    1. **Input Features**:
       - **WPB [kN]**: Web post-buckling force (39–4200).
       - **h [mm]**: Overall depth (220–1355).
       - **h0 [mm]**: Web opening height (138–1200).
       - **s [mm]**: Web opening spacing (60–1500).
       - **a0 [mm]**: Web opening width (34–780).
    2. **Validation**:
       - Ensure inputs are within valid ranges. Errors will be displayed if values are invalid.
    3. **Prediction**:
       - Click "Predict" after entering valid inputs to compute section properties.
       - Results include the nearest section dimensions and yield strength.
    4. **Visualization**:
       - View the structural visualization image below the results.
...
wpb = st.sidebar.number_input("WPB [kN]", min_value=wpb_min, max_value=wpb_max, step=1.0, value=400.0)
if wpb < wpb_min or wpb > wpb_max:
    st.sidebar.error(f"WPB must be between {wpb_min} and {wpb_max} kN.")
    error = True

h = st.sidebar.number_input("h [mm]", min_value=h_min, max_value=h_max, step=1.0, value=500.0)
if h < h_min or h > h_max:
    st.sidebar.error(f"h must be between {h_min} and {h_max} mm.")
    error = True

h0 = st.sidebar.number_input("h0 [mm]", min_value=h0_min, max_value=h0_max, step=1.0, value=150.0)
if h0 < h0_min or h0 > h0_max:
    st.sidebar.error(f"h0 must be between {h0_min} and {h0_max} mm.")
    error = True

s = st.sidebar.number_input("s [mm]", min_value=s_min, max_value=s_max, step=1.0, value=100.0)
if s < s_min or s > s_max:
    st.sidebar.error(f"s must be between {s_min} and {s_max} mm.")
    error = True

a0 = st.sidebar.number_input("a0 [mm]", min_value=a0_min, max_value=a0_max, step=1.0, value=50.0)
